...
export default function FriendRequests() {
  ...
  useEffect(() => {
    ...
    const unsubscribeIncoming = onSnapshot(
      query(requests, where("recipientId", "==", user.uid)),
      loadRequests,
    );
    const unsubscribeSent = onSnapshot(
      query(requests, where("senderId", "==", user.uid)),
      loadRequests,
    );
  ...
  const respondToRequest = async (request, accepted) => {
    ...
    try {
      ...
      if (accepted) {
        ...
        const previousFriend = request.reconnect
          ? await getDoc(previousFriendRef)
          : null;
      ...
      batch.set(
        doc(collection(db, "users", request.senderId, "notifications")),
        {
          message: accepted
            ? `${user.displayName || user.email || "A friend"} accepted your friend request.`
            : `${user.displayName || user.email || "A friend"} declined your friend request.`,
      ...
      setMessage("We could not update that friend request. Please try again.");
    }
  };
  ...
  const cancelRequest = async (request) => {
    try {
      ...
      setMessage("We could not cancel that friend request. Please try again.");
    }
  };
  ...
  return (
    <div className="dash-screen">
      <div className="page-card">
        ...
        {incoming.length === 0 && (
          <p className="muted">You do not have any friend requests yet.</p>
        )}
        <div className="requests-list">
          {incoming.map((request) => (
            <div className="request-row" key={request.id}>
              <div>
                <strong>{request.senderName || request.senderEmail}</strong>
        ...
        {sent.length === 0 && (
          <p className="muted">No pending friend requests sent.</p>
        )}
        <div className="requests-list">
          {sent.map((request) => (
            <div className="request-row" key={request.id}>
